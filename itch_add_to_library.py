...
csrf_token = login_soup.find('form', attrs={'method': 'POST'}).find('input', attrs={'name': 'csrf_token'}).get('value')
...
base_bundle_soup = bs4.BeautifulSoup(session.get(base_bundle_page).content)
...
while run:
    ...
    soup = bs4.BeautifulSoup(session.get(bundle_page, params={'page': page}).content, features='html5lib')
